    ...
    return pd.DataFrame(
        {
            ...
            "Deposit_Count": [2] * 40,
            "Rolling_7_Day_Average": pd.Series(range(1000, 1040)).rolling(7, min_periods=1).mean(),
            "Rolling_30_Day_Average": pd.Series(range(1000, 1040)).rolling(30, min_periods=1).mean(),
            "Lag_1_Day": pd.Series(range(1000, 1040)).shift(1),
            "Lag_7_Day": pd.Series(range(1000, 1040)).shift(7),
            "Lag_30_Day": pd.Series(range(1000, 1040)).shift(30),
            "Cash_Flow_Ratio": [0.5] * 40,
